...
class FMFirstOrderLinear(nn.Module):
    ...
    def embed_token_fields(self, token_fields, positions=None):
        ...
        raise ValueError(token_embedding.shape)
    ...
    def forward(self, interaction):
        ...
        if position is not None:
            ...
            for field_name in self.token_field_names:
                if field_name == self.positional_encoding["embed_field"]:
                    position_field = interaction[field_name].unsqueeze(1)
        ...
        if position is not None:
            ...
            raise ValueError(token_fields_embedding.shape)
            token_fields_embedding[:, 0, i] = self.positional_embedding(token_fields_embedding[:, 0, i], input_pos=position_field)
...
# Adapted from
# https://pytorch.org/torchtune/0.6/_modules/torchtune/modules/position_embeddings.html#RotaryPositionalEmbeddings
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # tensor has shape [b, d // 2, 2]
        xshaped = x.float().reshape(*x.shape[:-1], -1, 2)

        # reshape the cache for broadcasting
        # tensor has shape [b, d // 2, 2] if packed samples,
        # otherwise has shape [1, sd // 2, 2]
        rope_cache = rope_cache.view(-1, xshaped.size(1), 2)
